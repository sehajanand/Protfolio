import React from "react";
import "./Resumecontentsubbox2.css";

export default function Resumecontentsubbox2() {
  return (
    <div className="resume-half-two">
      <h2>Education</h2>
      <ul>
        <li>
          <em>Secondary Education</em>
          <br />
          Delhi Public School, Jammu <br />
          09/2013 - 04/2020
        </li>
        <li>
          <em>B Tech in Chemical Engineer </em> <br /> National Institute Of
          Technology, Srinagar <br />
          11/2020 - 09/2024
        </li>
      </ul>{" "}
      <h2>Work / Learning Experience</h2>
      <ul>
        <li>
          <em>Zoonotics Esports Organisation </em> <br /> Team Leader of Front
          end Website Development Team
        </li>
        <li>
          <em>Member Of IICHE (Indian Institute Of Chemical Engineers) </em>
          Organizing various iiche events by team collaboration and gaining vast knowledge directly from invited professors.
        </li>
      </ul>
      <h2>Interests</h2>
      <ul>
        <li>Football</li>
        <li>Chess</li>
        <li>Travelling</li>
      </ul>
    </div>
  );
}
